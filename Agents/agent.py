...
root_agent = LlmAgent(
    name="root_agent",
    model="gemini-2.0-flash",
    instruction="You are a root agent that manages interactions between the greeter and task executor agents. Route user requests to the appropriate sub-agent based on the nature of their request. If it's a greeting or introduction, use the greeter. If it's a task or request for action, use the task executor.",
